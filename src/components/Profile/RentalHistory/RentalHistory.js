...
const RentalHistory = () => {
        ...
        }, [load, unreadNotify, toggleStatus, houseName, status, localStartTime])
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    BookingService.cancelBooking(item.id, {message: result.value}).then((response) => {
                        ...
                        handleSendNotify(account, item.doctor.owner.id, `${account.username} đã hủy lịch khám. Lịch đặt: ${from}`, 'profile/houses-owner-booking')
                    }).catch(function (err) {
                        console.log(err);
                        setIsProgressing(false);
                    })
        ...
        const showCancelBookingConfirm = (booking) => {
            if (new Date(booking.startTime) - new Date() > (1000 * 60 * 60 * 24)) {
                Swal.fire({
                    title: 'Bạn chắc chắn muốn hủy khám?',
                    icon: 'warning',
                    showCancelButton: true,
                ...
                Swal.fire({
                    title: 'Thời gian hủy nhỏ hơn 1 ngày tính tới ngày đặt',
                    icon: 'error',
                    showCancelButton: true,
                    confirmButtonText: 'Xác nhận',
                    cancelButtonText: 'Đóng',
        ...
        const handleReview = (values) => {
            ...
            BookingService.createReview(data).then(response => {
                ...
                setLoad(!load);
                const doctor = response.data.booking.doctor;
                handleSendNotify(account, doctor.owner.id, `${account.username} đã bình luận về bạn.`, `house-detail/${doctor.id}`)
            }).catch(function (err) {
                console.log(err);
                Swal.fire({
                    icon: 'error',
