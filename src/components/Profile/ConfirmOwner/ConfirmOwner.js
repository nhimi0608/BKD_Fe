...
const ConfirmOwner = () => {
    ...
    const handleAgree = (value) => {
        ...
        Swal.fire({
            title: `Bạn đồng để ${value.lastname} ${value.firstname} làm bác sĩ chứ?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                AccountService.agreeRegister(data).then((response) => {
                    ...
                    setLoad(!load);
                    handleSendNotify(account, value.account.id, 'Admin đã đồng ý cho bạn làm bác sĩ', 'profile/houses-owner');
                }).catch(function (err) {
                    console.log(err);
                })
            }
    ...
    const handleRefuse = (data) => {
        Swal.fire({
            title: `Bạn từ chối để ${data.lastname} ${data.firstname} làm bác sĩ?`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: 'Xác nhận',
            cancelButtonText: 'Đóng',
            ...
            if (result.isConfirmed) {
                AccountService.refuseRegister(data.id).then((response) => {
                    ...
                    setLoad(!load);
                    handleSendNotify(account, data.account.id, 'Admin đã từ chối cho bạn làm bác sĩ', 'profile/register-owner');
                }).catch(function (err) {
                    console.log(err);
                })
            }
    ...
    return (
        <div className={'col-9'}>
            <h3 className="text-uppercase mb-4 text-center"> Danh sách đăng ký làm bác sĩ</h3>
