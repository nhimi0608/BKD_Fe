...
const ListDoctor = () => {
    ...
    const findAllDoctors = (roleName, nameSearch, status, currentPage) => {
        AccountService.findAllDoctors(currentPage)
            .then((accounts) => {
    ...
    useEffect(() => {
        findAllDoctors(currentPage - 1);
        window.scrollTo({
            top: 0,
            behavior: "smooth"
        })
    }, [currentPage, currentPageMd, nameSearch, revenue, doctor, status, isLoad])
    ...
    return (
        <div className="col-9">
            <h3 className="text-uppercase text-center mb-5">Danh sách bác sĩ trong hệ thống</h3>
